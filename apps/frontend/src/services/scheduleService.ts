...
export function getScheduleStorageKey(userId?: string, weekStart?: string) {
  return `gym-web.schedule.${userId || "guest"}.${weekStart || "current"}`;
}

export function loadWeeklySchedule(userId?: string, weekStart?: string): WeeklySchedule {
  const fallback = createEmptySchedule();
  const rawSchedule =
    localStorage.getItem(getScheduleStorageKey(userId, weekStart)) ??
    localStorage.getItem(getScheduleStorageKey(userId));
...
export function saveWeeklySchedule(
  ...
  localStorage.setItem(getScheduleStorageKey(userId, weekStart), JSON.stringify(schedule));
}
